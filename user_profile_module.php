...
// ✅ Allow only 'user' role (NOT 'admin' or 'dentist')
...
// Fetch billing records to check payment amounts
$billing_sql = "SELECT appointment_id, total_amount, payment_status FROM billing";
...
$apptSql = "SELECT a.Appointment_ID, a.Appointment_Date, a.Service_ID as Service_Type, d.name AS dentist_name, a.Appointment_Status, a.notes 
            FROM appointment a
...
// Function to get total amount for appointment
function getTotalAmount($appointment_id, $billing_info) {
    if (isset($billing_info[$appointment_id])) {
        return $billing_info[$appointment_id]['total_amount'];
    }
    return 0;
}

// Function to get payment status for appointment
function getPaymentStatus($appointment_id, $billing_info) {
    if (isset($billing_info[$appointment_id])) {
        return $billing_info[$appointment_id]['payment_status'];
    }
    return 'Unpaid';
}
...
<head>
    ...
    <link rel="stylesheet" href="user_profile_module.css">
    <style>
    ...
    <div class="tabbed-layout">
        ...
        <div class="content-column">
            ...
            <div id="personal" class="content-section active">
                ...
                <?php endif; ?>
                <form id="personalForm" method="POST" action="update_user_profile.php">
                    ...
                    <tbody>
                        <?php if (!empty($appointments)): ?>
                            <?php foreach ($appointments as $appt): ?>
                                <?php
                                    // Get service names using the function
                                    $service_names = getServiceNamesFromIDs($conn, $appt['Service_Type']);
                                    $service_display = !empty($service_names) ? implode(", ", $service_names) : 'N/A';
                                    $formatted_date = date('m/d/Y', strtotime($appt['Appointment_Date']));
                                    
                                    // Get amount and payment status
                                    $appointment_id = $appt['Appointment_ID'];
                                    $total_amount = getTotalAmount($appointment_id, $billing_info);
                                    $payment_status = getPaymentStatus($appointment_id, $billing_info);
                                    
                                    // Determine CSS class based on payment status
                                    $amount_class = 'amount-' . strtolower($payment_status);
                                ?>
                                <tr>
